...
function ideal_V(path::String)
	...
	function print_ideal_v(file, dict::Array{Any}, params::Video_Params,
	    ...
	    for v = 1:num_videos
	        ...
	        for f = 1:num_frames
				...
				alphas, betas = update_alpha_beta(false, alphas, betas, objects_observed[v,f], gt_objects_2D, params)
	        end
	...
	gt_v[:,1] = (alphas[:,1] .- 1) ./ (betas[:,1] .- 1) #had been ((betas[:,1] .- 1).^2) in denominator
