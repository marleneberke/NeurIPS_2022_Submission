...
#Get ground-truth bounding boxes for the objects in the scenes and save in json
...
from tdw.output_data import OutputData, Bounds, Images, Collision, EnvironmentCollision, SceneRegions, SegmentationColors
...
min_n_objects = 1 #1
max_n_objects = 4 #4. never samples this top number
...
# objects = dict({'chair': ['chair_thonet_marshall'],
# 'plant': ['monstera_plant'],
# 'tv': ['samsungtv'],
# 'umbrella': ['umbrella_fix'],
# 'bowl': ['round_bowl_large_metal_perf']})
...
for video in tqdm(range(num_videos)):
    ...
    c.communicate({"$type": "set_render_quality", "render_quality": 5})
    ...
    num_objects = randrange(min_n_objects, max_n_objects)
    ...
    for frame, (camera, lookat) in zip(range(num_frames), trajectories):
        ...
        ########################################################################
        #Image and segmentation stuff

        labels2D = [] #going to be a list of dictionaries
        ...
        for r in resp:
            ...
            if r_id == "segm":
                segm = SegmentationColors(r)
                for j in range(segm.get_num()):
                    object_id = segm.get_object_id(j)
                    segmentation_color = segm.get_object_color(j)
                    segmentation_colors[object_id] = segmentation_color

        TDWUtils.save_images(images, filename=f"{video:03}_{frame:03}")

        print(f"segmentation_colors {segmentation_colors}")
        for object_id in segmentation_colors:
            segmentation_color = segmentation_colors[object_id]
            object_cat = category_names[object_id]
            #if (im==segmentation_color).any(): #if there's at least a pixel with the segmentation color
            Y, X = np.where(np.all(im==segmentation_color, axis=2)) #X and Y were flipped
            if len(X) > 0 and len(Y) > 0:
                top_left = [float(min(X)), float(min(Y))]
                bottom_right = [float(max(X)), float(max(Y))]
                center = [(top_left[0] + bottom_right[0])/2, (top_left[1] + bottom_right[1])/2]
                labels2D.append({"category_name": object_cat, "top_left": top_left,
                                "bottom_right": bottom_right, "center": center})

        views.append({"camera": camera, "lookat": lookat, "labels2D": labels2D})
